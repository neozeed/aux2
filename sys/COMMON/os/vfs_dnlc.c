...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 2.1 89/10/13 19:27:15}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1989\tVersion 2.1 of vfs_dnlc.c on 89/10/13 19:27:15";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
#define	NC_HASH(namep, namlen, vp)	\
	((name[0] + namlen + (int) vp) & (NC_HASH_SIZE-1))
	...
	struct ucred *cred;
{
	...
	if (ncp = dnlc_search(dp, name, namlen, hash, cred)) {
		ncstats.dbl_enters++;
		return;
	}
	/*
	...
	struct ucred *cred;
{
	...
	hash = NC_HASH(name, namlen, dp);
	if ((ncp = dnlc_search(dp, name, namlen, hash, cred)) == (struct ncache *) 0) {
		ncstats.misses++;
		return ((struct vnode *) 0);
	}
	ncstats.hits++;
	...
	register struct ucred *cred;
{
	...
	nhp = &nc_hash[hash];
	for (ncp = nhp->hash_next; ncp != (struct ncache *)nhp; ncp = ncp->hash_next) {
		if (ncp->dp == dp &&
		    *ncp->name == *name &&	/* fast chk for 1st chr match */
		    ncp->namlen == namlen &&
