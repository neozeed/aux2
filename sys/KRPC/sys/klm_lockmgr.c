/* @(#)klm_lockmgr.c	1.1 87/06/30 3.2/4.3NFSSRC */
/*	@(#)klm_lockmgr.c 1.1 86/09/25 ;Copyright 1986 Sun Microsystems	*/
...
requestloop:
	...
	case KLM_LOCK:
		...
		case klm_denied:
			if (args.block) {
				/* blocking requests can only be
				   denied if we would cause a deadlock */
				error = EDEADLK;
				goto ereturn;
			}
			error = EACCES;		/* EAGAIN?? */
...
/*NOTREACHED*/
nolocks_wait:
	timeout(wakeup, (caddr_t)&lm_sa, (backoff_timeout * v.v_hz));
