#ifndef lint	/* .../sys/NFS/sys/nfs_vnodeops.c */
#define _AC_NAME nfs_vnodeops_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 2.2 90/02/19 11:25:03}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 2.2 of nfs_vnodeops.c on 90/02/19 11:25:03";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
/*	@(#)nfs_vnodeops.c	1.6 of 2/20/87 UniSoft */
	...
	struct ucred *cred;
{
	...
	/*
	 * Add compatability checks on signals. HNK
	 */
	...
	/*
	 * Add "if" for not BSD compatible state. HNK
	 */
	if (rw == UIO_WRITE && vp->v_type == VREG)
		if ((uio->uio_offset >> 9) >= u.u_limit)
			return (EFBIG);
		else
			if ((uio->uio_offset + uio->uio_resid) >> 9 > u.u_limit)				uio->uio_resid = (u.u_limit - (uio->uio_offset >> 9)) << 9;
	rp = vtor(vp);
	size = vtoblksz(vp);
	size &= ~(DEV_BSIZE - 1);
	if (size <= 0) {
	...
	/*  This fix keeps lockf from causing nfs to read past eof  */
	...
	struct ucred *cred;
{
	...
	if (!error) {
		nfs_attrcache(vp, &ns->ns_attr, NOFLUSH, 0);
	}
	...
	struct ucred *cred;
{
	...
	*residp = count;
	...
	if (!error) {
		nfs_attrcache(vp, &rr.rr_attr, SFLUSH, 0);
	}
#ifdef NFSDEBUG
...
nfs_attrcache(vp, na, fflag, create_flag)
	struct vnode *vp;
	struct nfsfattr *na;
	...
	if (timercmp(&time, &rp->r_nfsattrtime, <)) {
		...
		if (!error) {
			...
			if (!error) {
				...
				nfs_attrcache(vp, &ns->ns_attr, SFLUSH, 0);
			} else {
				check_stale_fh(error, vp);
			}
		}
	...
	struct ucred *cred;
{
	...
	*vpp = (struct vnode *) dnlc_lookup(dvp, nm, cred);
	...
	if (!error && ISVDEV((*vpp)->v_type)) {
		...
		newvp = (struct vnode *)specvp(*vpp, (*vpp)->v_rdev);
		VN_RELE(*vpp);
		*vpp = newvp;
	}
#endif
...
#define	IFSOCK		0140000		/* socket */
	...
	if (!error) {
		...
		if (!error) {
			*vpp = makenfsnode(
			    &dr->dr_fhandle, &dr->dr_attr, dvp->v_vfsp, 1);
			if (va->va_size == 0) {
				(vtor(*vpp))->r_size = 0;
			}
