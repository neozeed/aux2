#ifndef lint	/* .../sys/NFS/sys/nfs_subr.c */
#define _AC_NAME nfs_subr_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 2.2 90/02/19 11:24:44}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 2.2 of nfs_subr.c on 90/02/19 11:24:44";
#endif		/* _AC_HISTORY */
#endif		/* lint */
 ...
 *  I declare it in rpc/clnt.h to make this file compile! HNK
 *
 * #define	RPC_INTR	18
 */
...
#ifdef notdef
		if (u.u_ttyp && u.u_ttyd != cons.t_index) {
...
int rnode_reuse, rnode_new, rnode_active;
 ...
 */
struct vnode *
makenfsnode(fh, attr, vfsp, create_flag)
	fhandle_t *fh;
	...
	for (i = 0; i < RTABLESIZE; i++) {
		for (rp = rtable[i]; rp; ) {
			if (rp->r_vnode.v_vfsp == vfsp) {
				 ...
				 * it which will call rnode_free if the ref 
				 * count goes to zero and rnode_free will 
				 * mess up rtable.
				 */
				VN_HOLD(rtov(rp));
				rptmp = rp;
...
nfstsize()
{
	...
	register int tsize = NFS_MAXDATA;

	 for (ifp = ifnet; ifp; ifp = ifp->if_next)
	     if (ifp->if_name[0] == 'a' && ifp->if_name[1] == 'e')
		 tsize = min(tsize, 8192);
#ifdef NFSDEBUG
