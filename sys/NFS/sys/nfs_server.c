...
#define _AC_NO_MAIN "@(#) Copyright (c) 1983-87 Sun Microsystems Inc., 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 2.2 90/02/19 11:27:14}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 2.2 of nfs_server.c on 90/02/19 11:27:14";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	register struct nfsrddirres  *rd;
{
	...
	/*
	 * This is a horrible hack. If you add another file
	 * system, you will need a switch here. Or you could
	 * re-design the svfs code to return the directory data
	 * is some different data format. pjh@usl
	 */
	if(vp->v_vfsp->vfs_op == vfssw[MOUNT_SVFS]) {
		issvfs = 1;
	}
...
nxtblk:
	 ...
	 * Rd_bufsize holds the amount of data requested by the client.
	 * Rd_size is the amount of data read in by VOP_READDIR.  Rd_size
	 * bytes will be freed by rfs_rddirfree.
	 */
	...
	if (uio.uio_resid) {
		...
		/*
		 * What a crock, we need to free the tail end of the buffer because rd_size,
		 * and rd_bufsize are used in the xdr_rddirres routine for bounds
		 * checks.  This means that the information about the size of the buffer
		 * is lost...
		 */
		kmem_free((caddr_t)((int)rd->rd_entries + rd->rd_bufsize - uio.uio_resid), uio.uio_resid);
		rd->rd_eof = TRUE;
	} else {
		rd->rd_eof = FALSE;
	}
	...
	if(issvfs) {
		while ((skipped < rd->rd_size) &&
		       ((offset + sizeof(struct svfsdirect) <= rda->rda_offset) ||
			(dp->d_ino == 0))) {
		    skipped += dp->d_reclen;
...
bad:
	/*
	 * Mark the data, so that xdr_putrddirres can work
	 * out the offsets.  This is horrible, but will be
	 * fixed with the next release of NFS.
	 */
	if(issvfs)
	...
	struct nfsrddirres *rd;
{
	if (rd->rd_entries != NULL)
	    kmem_free((caddr_t)rd->rd_entries, (u_int)rd->rd_size);
}
	...
	register SVCXPRT *xprt;
{
	...
	if (which != RFS_NULL) {
		...
		for (i=0; i < aup->aup_len; i++)
			newcr->cr_groups[i] = aup->aup_gids[i];
		for (; i < NGROUPS; i++)
