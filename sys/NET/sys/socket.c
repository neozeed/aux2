...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 2.2 90/04/02 09:14:47}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 2.2 of socket.c on 90/04/02 09:14:47";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
restart:
	...
	do {
		...
		while (space > 0) {
			...
			if (uio->uio_resid >= MCLBYTES / 2 && space >= MCLBYTES) {
				MCLGET(m);
				if (m->m_len != MCLBYTES)
					goto nopages;
