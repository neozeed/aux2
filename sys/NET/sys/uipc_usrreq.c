#ifndef lint	/* .../sys/NET/sys/uipc_usrreq.c */
#define _AC_NAME uipc_usrreq_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1980-87 The Regents of the University of California, All Rights Reserved.  {Apple version 2.2 89/10/16 09:38:00}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.2 of uipc_usrreq.c on 87/11/06 13:05:26";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
ino_tl	unp_vno;			/* prototype for fake vnode numbers */
...
#undef rcv
	...
	case PRU_SENSE:
		...
		if (unp->unp_vno == 0)
			unp->unp_vno = unp_vno++;
		((struct stat *) m)->st_ino = (ino_t)unp->unp_vno;
		((struct stat *) m)->st_inol = unp->unp_vno;
		return (0);
...
/* SHOULD BE ABLE TO ADOPT EXISTING AND wakeup() ALA FIFO's */
	...
	unp->unp_vnode = vp;
	/*
	 * Since we don't have vnode locking and 4.3
	 * relied upon inode locking, this statement
	 * must be done last
	 */
	...
	struct mbuf *nam;
{
	...
	*(mtod(nam, caddr_t) + nam->m_len) = 0;
	...
	if (error)
		return (error);
	if (error = VOP_ACCESS(vp, VWRITE, u.u_cred)) {
		goto bad;
	}
	if (vp->v_type != VSOCK) {
		error = ENOTSOCK;
