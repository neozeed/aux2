...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 2.1 89/10/14 14:48:58}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 2.1 of in_pcb.c on 89/10/14 14:48:58";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	struct mbuf *nam;
{
	...
	if (in_ifaddr) {
		 ...
		 * is not the loop device and a broadcast interface
		 * if one exists.
		 */
		for (ia = in_ifaddr, savia = NULL; ia; ia = ia->ia_next) {
	...
	int flags;
{
	...
	for (inp = head->inp_next; inp != head; inp = inp->inp_next) {
		...
		if ((inp->inp_faddr.s_addr != INADDR_ANY) && 
			(in_broadcast(inp->inp_faddr.s_addr) == 0)) {
